...
class AmortizedAccountant(object):
  ...
  def accumulate_privacy_spending(self, eps_delta, unused_sigma,
    ...
    with tf.control_dependencies(
      ...
      # and proof.
      amortize_eps = tf.reshape(tf.compat.v1.log(1.0 + amortize_ratio * (
          tf.exp(eps) - 1.0)), [1])
...
class MomentsAccountant(object):
  ...
  def get_privacy_spent(self, sess, target_eps=None, target_deltas=np.arange(0.99,1)):
  ...
  def _compute_log_moment(self, sigma, q, moment_order):
    ...
    qs = tf.exp(tf.constant([i * 1.0 for i in range(moment_order + 1)],
                            dtype=tf.float64) * tf.cast(
                                tf.compat.v1.log(q), dtype=tf.float64))
    ...
    return tf.squeeze(tf.compat.v1.log(tf.cast(q * term0 + (1.0 - q) * term1,
                                     tf.float64)))
