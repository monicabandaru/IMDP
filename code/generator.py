...
class Generator():
    def __init__(self, n_node, node_emd_init, config):
        ...
        with tf.compat.v1.variable_scope('generator',reuse=tf.compat.v1.AUTO_REUSE):
            if node_emd_init:
                self.node_embedding_matrix = tf.compat.v1.get_variable(name='gen_node_embedding',
            ...
            self.gen_w_1 = tf.compat.v1.get_variable(name='gen_w',
                                                 shape=[2, self.emd_dim, self.emd_dim],
                                                 initializer=tf.keras.initializers.GlorotNormal(),
                                                 trainable=True)
            self.gen_b_1 = tf.compat.v1.get_variable(name='gen_b',
                                                 shape=[2, self.emd_dim],
                                                 initializer=tf.keras.initializers.GlorotNormal(),
                                                 trainable=True)
            self.gen_w_2 = tf.compat.v1.get_variable(name='gen_w_2',
                                                 shape=[2, self.emd_dim, self.emd_dim],
                                                 initializer=tf.keras.initializers.GlorotNormal(),
                                                 trainable=True)
            self.gen_b_2 = tf.compat.v1.get_variable(name='gen_b_2',
                                                 shape=[2, self.emd_dim],
                                                 initializer=tf.keras.initializers.GlorotNormal(),
                                                 trainable=True)

            tf.compat.v1.disable_eager_execution()
            self.node_ids = tf.compat.v1.placeholder(tf.int32, shape=[None])

            self.noise_embedding = tf.compat.v1.placeholder(tf.float32, shape=[2, None, self.emd_dim])

            self.dis_node_embedding = tf.compat.v1.placeholder(tf.float32, shape=[2, None, self.emd_dim])
            tf.compat.v1.disable_eager_execution()
